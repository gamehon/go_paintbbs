...
Neo.uploadGo = function(board, blob){
  var fileInput = document.getElementById("nf");
  var formData = new FormData();
  formData.append("nf" , blob);
  ...
  xhr.onload = function (e) {
    ...
    var errorMessage = null;
    if (xhr.status / 100 != 2) {
      errorMessage = xhr.responseURL + "\n"
                   + Neo.translate("投稿に失敗。時間を置いて再度投稿してみてください。");
    } else if (xhr.response.match(/^error\n/m)) {
      errorMessage = xhr.response.replace(/^error\n/m, '');
    } else {
      Neo.uploaded = true;
    }
    ...
    if (document.paintBBSCallback) {
      document.paintBBSCallback(jsonResponse["name"]);
    }
  }
  xhr.send(formData);
}
